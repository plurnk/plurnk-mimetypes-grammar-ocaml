#!/usr/bin/env node
// CI byte-identical reproducibility check for ocaml.wasm.
...
const committed = await readFile(path.join(repoRoot, "ocaml.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed ocaml.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-ocaml-verify-"));
...
console.log(`rebuilt ocaml.wasm sha256: ${rebuiltHash}`);
