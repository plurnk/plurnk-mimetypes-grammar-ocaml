#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-ocaml.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-ocaml-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-ocaml.git", "src"], { cwd: work });
...
const buildCwd = path.join(work, "src", "grammars/ocaml");
await run(cli, ["generate"], { cwd: buildCwd });
await run(cli, ["build", "--wasm"], { cwd: buildCwd });

// Locate produced wasm and copy as ocaml.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "ocaml.wasm"));
const bytes = (await readFile(path.join(repoRoot, "ocaml.wasm"))).length;
console.log(`ocaml.wasm: ${bytes} bytes (built from ${pin})`);
